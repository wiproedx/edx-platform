...
def set_credit_requirement_status(user, course_key, req_namespace, req_name, status="satisfied", reason=None):
    ...
    """
    # Check whether user has verified enrollment. Only those should be eligible for the credit.
    enrollment_mode, is_active = CourseEnrollment.enrollment_mode_for_user(user, course_key)
    has_verified_enrollment = (CourseMode.is_verified_slug(enrollment_mode) and is_active)

    # Refuse to set status of requirement if the user has non-verified enrollment.
    if not has_verified_enrollment:
        return
